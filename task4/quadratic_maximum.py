...
# 启用LaTeX渲染数学公式
plt.rcParams['text.usetex'] = False  # 保持False避免依赖LaTeX环境，使用matplotlib内置渲染
plt.rcParams['mathtext.fontset'] = 'cm'  # 使用Computer Modern字体渲染数学符号

# 创建图形
fig = plt.figure(figsize=(PLOT_WIDTH, PLOT_HEIGHT))
fig.suptitle('二次函数极大值求解：$y = -x^2 + 4x + 5$', fontsize=16, fontweight='bold')
...
# 绘制函数曲线 - 修改图例为TeX语法
ax1.plot(x_vals, y_vals, 'b-', linewidth=3, label='$y = -x^2 + 4x + 5$', alpha=0.8)
...
# 绘制函数曲线 - 修改图例为TeX语法
ax5.plot(x_vals, y_vals, 'b-', linewidth=2, alpha=0.5, label='$y = -x^2 + 4x + 5$')
